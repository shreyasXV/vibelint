...
exports.extractDeletedSymbols = extractDeletedSymbols;
exports.checkRemovedCode = checkRemovedCode;
// Extract deleted function/class names from a diff patch
function extractDeletedSymbols(file) {
    const symbols = [];
...
function checkRemovedCode(deletedFile, allFileContents, // filename -> content (rest of codebase)
config) {
    const issues = [];
    ...
    for (const symbol of deletedSymbols) {
        ...
        if (symbol.name.startsWith('_') && symbol.type === 'function')
            continue;
        // Skip very common names that would false-positive
        if (['setup', 'teardown', 'main', 'init', 'run', 'test', 'new', 'drop'].includes(symbol.name.toLowerCase()))
            continue;
        const references = [];
        for (const [filename, content] of allFileContents) {
            // Don't check the file where the symbol was deleted
            if (filename === deletedFile.filename)
                continue;
...
//# sourceMappingURL=removed-code.js.map